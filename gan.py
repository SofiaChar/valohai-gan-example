...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        M = (d_loss_real.item() + torch.abs(diff)).item()
        ...
        logs = {
            ...
            'd_loss': d_loss.item(),
            'g_loss': g_loss.item(),
            'convergence_metric': M
        }
        print(json.dumps(logs))  # Log to Valohai
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], output_path + "%d.png" % batches_done, nrow=5, normalize=True)
    ...
    torch.save(discriminator.state_dict(), output_path + 'discriminator.pth')
